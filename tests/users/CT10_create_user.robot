...
${endpoint}     /user/
${mensagem_erro_cpf_vazio}    O campo CPF é obrigatório!

*** Test Cases ***
Criar Usuário com CPF Vazio
    [Documentation]   Testa a criação de um usuário com o campo CPF vazio e verifica se a API retorna erro 400.
    Criar Sessao
    Realizar Login
    Criar Novo Usuario Com CPF Vazio

*** Keywords ***
Criar Novo Usuario Com CPF Vazio
    [Documentation]    Envia uma requisição POST para criar um novo usuário com o campo CPF vazio
    ${user_data}=  Generate User Data
    ${user_data["cpf"]}=  Set Variable  ${user_data["emptyString"]}  # Define CPF como vazio
    ...
    Should Be Equal As Strings    ${error_msg[0]}    ${mensagem_erro_cpf_vazio}
