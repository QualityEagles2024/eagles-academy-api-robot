...
${endpoint}     /user/
${mensagem_erro_confirm_password}    O campo de confirmação de senha é obrigatório.

*** Test Cases ***
Criar Usuário com ConfirmPassword Vazio
    [Documentation]   Testa a criação de um usuário com o campo confirmPassword vazio e verifica se a API retorna erro 400.
    Criar Sessao
    Realizar Login
    Criar Novo Usuario Com ConfirmPassword Vazio

*** Keywords ***
Criar Novo Usuario Com ConfirmPassword Vazio
    [Documentation]    Envia uma requisição POST para criar um novo usuário com o campo confirmPassword vazio
    ${user_data}=  Generate User Data
    ${user_data["confirmPassword"]}=  Set Variable  ${user_data["emptyString"]}  # Define confirmPassword como vazio
    ...
    Should Be Equal As Strings    ${error_msg[0]}    ${mensagem_erro_confirm_password}
